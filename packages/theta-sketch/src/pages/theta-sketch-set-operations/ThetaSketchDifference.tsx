...
const NARRATION: Record<number, string> = {
    ...
    1: `For difference, we use the operation threshold theta equals min of theta A and theta B, and estimate with m divided by theta.`,
    ...
    6: `We add the values, which are below the computed theta, from Sketch A and Sketch B into the difference result sketch.`,
    7: `We keep only values that are in Sketch A but not in Sketch B, below the computed theta.`,
    8: `Finally, we store the computed theta in the result and compute the estimate.`,
    9: `Because theta is stored explicitly, this difference result is composable for further set operations like union or intersection.`,
};
    ...
    React.useEffect(() => {
        ...
        const timelineScene: TimelineSceneThree = {
            ...
            timeline: [
                ...
                at(NARRATION_START[6] ?? 6).animate(latexDifference.latexId, { position: { y: `+=${window.innerHeight}` } }, { duration: 1 }),
                ...
                at(NARRATION_START[8] ?? 8).animate(latexDifference.latexId, { scale: { x: 1, y: 1, z: 1 } }, { duration: 1 }),
            ],
        };
...
export default function ThetaSketchDifference() {
    ...
    const difference = React.useMemo(
        () => ({ values: [0.04, 0.09, 0.18, 0.31, 0.39, 0.47, 0.58], theta: 0.72, estimated: 9.72 }),
        []
    );
