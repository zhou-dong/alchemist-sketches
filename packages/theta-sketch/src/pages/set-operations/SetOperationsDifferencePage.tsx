                ...
                <Stack spacing={3}>
                    ...
                    >
                        ...
                        <Typography variant="body2" color="text.secondary" sx={{ mb: 2, lineHeight: 1.7 }}>
                            For difference we need a <strong>common threshold</strong>: we only look at values <strong>below θ</strong> in both sketches, then count values that are in A but <strong>not</strong> in B. That count, divided by θ, gives the estimated difference size. So <strong>θ defines the operation</strong>. We use <strong>θ = min(θ_A, θ_B)</strong> so both sketches are on the same scale. The result cannot stay KMV for further set ops — see “New θ” and the limit below.
                        </Typography>
                        <Typography variant="body2" sx={{ fontWeight: 600, color: 'text.primary' }}>
                            The problem: KMV does not store θ. After we compute the difference, the result has no stored θ — we hit the same limit as with intersection.
                        </Typography>
                    </Paper>
                            ...
                            {visualStep >= 1 && (
                                <Fade in={true}>
                                    <Typography variant="body2" color="text.secondary" sx={{ fontFamily: 'monospace' }}>
                                        Common θ for difference = min(θ_A, θ_B) = <strong>{commonTheta.toFixed(3)}</strong>. Shaded region = “below θ”. We count values in A but not in B.
                                    </Typography>
                                </Fade>
                            )}
                            {visualStep >= 1 && (
                                <Stack direction="row" spacing={1} flexWrap="wrap" sx={{ gap: 1 }}>
                                    ...
                                    {visualStep >= 2 && (
                                        <CountBadge label="|A ∖ B|" count={difference.values.length} color={RESULT_COLOR} />
                                    )}
                                </Stack>
                            )}
                            {visualStep >= 2 && (
                    ...
                    >
                        ...
                        <Typography variant="body2" color="text.secondary" sx={{ mb: 1.5, lineHeight: 1.7 }}>
                            KMV stores only the K values — it never stores θ. So the difference result above is just a set of values (often fewer than K). We have <strong>no place to store</strong> “θ = min(θ_A, θ_B)” in that result. If we use that result in another intersection or difference, we would have to guess θ from the result (e.g. max(result values)), which loses the correct threshold. That is the <strong>limit of KMV</strong>.
                        </Typography>
                        <Typography variant="subtitle2" sx={{ fontWeight: 600, color: 'text.primary', mb: 1 }}>
                            Why we need θ
                        </Typography>
                        <Typography variant="body2" color="text.secondary" sx={{ lineHeight: 1.7 }}>
                            To fix this we need to <strong>save θ</strong> in the result. The correct θ is min(θ_A, θ_B). Once we save θ explicitly with the values below it, the result becomes a <strong>Theta Sketch</strong>. So <strong>θ is necessary</strong> for the result to participate in further set operations.
                        </Typography>
                    </Paper>
